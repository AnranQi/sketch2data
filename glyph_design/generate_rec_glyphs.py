...
def generate(save_glyph_dir, params_global, params, num_cls_imgs):
    global count
    ...
    for i in range(num_cls_imgs):
        ...
        save_name = os.path.join(save_glyph_dir,  "id_" + str(i) + "_" + labels[0] + "_" + labels[1] + "_" + labels[2]  + ".svg")
...
def main():
    ...
    params_global['MIN_TRANSLATE_X'] = -1  # here 25 is the margin for min x border
    params_global['MAX_TRANSLATE_X'] = 1  # here 75 is the margin for max x border
    params_global['MIN_TRANSLATE_Y'] = -1
    params_global['MAX_TRANSLATE_Y'] = 1  # here 75 is the margin for max x border
    params_global['PRESERVE_RATIO'] = True  # bool
    params_global['PER_POINT_NOISE'] = 0  # absolute translation distance in svg units

    save_glyph_dir = ".\\glyph_rec\\thoughts_rec_dataset"

    if not os.path.exists(save_glyph_dir):
        os.makedirs(save_glyph_dir)

    generate(save_glyph_dir, params_global, params, num_cls_imgs=200)
